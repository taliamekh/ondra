...
/** Map a WMO weather code to a simple condition + emoji + label. */
function codeInfo(code: number): { condition: string; emoji: string; label: string } {
  if (code === 0) return { condition: 'clear', emoji: '☀️', label: 'Clear' };
  if (code <= 2) return { condition: 'clouds', emoji: '🌤️', label: 'Partly cloudy' };
  if (code === 3) return { condition: 'clouds', emoji: '☁️', label: 'Overcast' };
  ...
  return { condition: 'thunder', emoji: '⛈️', label: 'Thunderstorm' };
}
